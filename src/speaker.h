/**
 * @file speaker.h
 * @brief Speaker Driver for MicroMouse KERISE
...
public:
  ...
  bool begin(gpio_num_t gpio_num, ledc_timer_t timer, ledc_channel_t channel) {
    channel_ = channel;
    timer_ = timer;
    // LEDC Timer
...
protected:
  ...
  void write_note(note_t note, uint8_t octave) {
    ...
    ESP_ERROR_CHECK(ledc_set_duty(mode_, channel_, duty_));  //< duty in [0, 2^duty_resolution]
